...
class AssiseManager {
    ...
    updateGlobalAssiseList() {
            ...
            if (assisesForType && assisesForType.size > 0) {
                for (const [index, assise] of assisesForType.entries()) {
                    allAssises.push({
                        ...
                        elementCount: this.getNonJointElementCountForType(type, index),
        ...
        // NOUVEAU: Filtrer pour ne garder que les assises avec condition REMPLIE
        const filledAssises = allAssises.filter(assise => assise.elementCount > 0);

        // G�n�rer le HTML
        globalAssiseList.innerHTML = '';
        
        if (filledAssises.length === 0) {
            globalAssiseList.innerHTML = '<div class="empty-list">Aucune assise remplie</div>';
            return;
        }

        filledAssises.forEach(({ type, index, assise, height, elementCount, isActive, jointHeight }) => {
            ...
            item.innerHTML = `
                <div class="assise-item-info">
                    ...
                    <div class="assise-item-details">
                        ...
                        <div>${elementCount} �l�ment${elementCount !== 1 ? 's' : ''}</div>
                    </div>
                </div>
